<!DOCTYPE html>
<html lang="pt-BR">
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>Home - Funcionário | NailSpot</title>
  <link rel="stylesheet" href="css/funcionario-home.css">
  <link rel="shortcut icon" href="img/LogoNailspotofc.png" >
</head>
<body>
  <!-- CABEÇALHO -->
  <header class="navbar">
    <div class="logo">
      <img src="img/LogoNailspot+.png" alt="Logo NailSpot">
      <span class="nome-logo">NailSpot</span>
    </div>
    <nav>
      <ul>
        <li><a href="funcionario-home.html" class="ativo">Home</a></li>
        <li><a href="funcionario-perfil.html">Perfil</a></li>
      </ul>
    </nav>
  </header>

  <main class="conteudo">
    <h1>Meus Atendimentos</h1>
    <!-- Seleção de serviços oferecidos -->
<section class="card selecao-servicos">
  <h2>Serviços Oferecidos</h2>
  <form id="form-servicos">
    <label>
      <input type="checkbox" id="manicure" checked>
      Manicure
    </label>
    <label>
      <input type="checkbox" id="pedicure">
      Pedicure
    </label>
    <button type="submit" class="btn-principal">Salvar Preferências</button>
  </form>
</section>

    <section class="card">
      <div class="atendimento">
        <div class="info">
          <p><strong>Cliente:</strong> Ana Souza</p>
          <p><strong>Serviço:</strong> Manicure</p>
          <p><strong>Data:</strong> 10/11/2025</p>
          <p><strong>Hora:</strong> 14:00</p>
        </div>
        <div class="acoes">
          <button class="btn-remarcar">Remarcar</button>
          <button class="btn-cancelar">Cancelar</button>
        </div>
      </div>

      <div class="atendimento">
        <div class="info">
          <p><strong>Cliente:</strong> Júlia Ramos</p>
          <p><strong>Serviço:</strong> Pedicure</p>
          <p><strong>Data:</strong> 11/11/2025</p>
          <p><strong>Hora:</strong> 16:30</p>
        </div>
        <div class="acoes">
          <button class="btn-remarcar">Remarcar</button>
          <button class="btn-cancelar">Cancelar</button>
        </div>
      </div>
    </section>
  </main>

  <!-- MODAL REMARCAR -->
  <div id="modal-remarcar" class="modal">
    <div class="modal-conteudo">
      <h3>Remarcar Atendimento</h3>
      <form id="form-remarcar">
        <label for="data">Nova Data:</label>
        <input type="date" id="data" required>

        <label for="hora">Nova Hora:</label>
        <select id="hora" required>
          <option value="">Selecione</option>
          <option>10:00</option>
          <option>11:00</option>
          <option>14:00</option>
          <option>16:00</option>
        </select>

        <div class="botoes-modal">
          <button type="submit" class="btn-principal">Confirmar</button>
          <button type="button" class="btn-cancelar" id="fechar-remarcar">Voltar</button>
        </div>
      </form>
    </div>
  </div>

  <!-- MODAL CANCELAR -->
  <div id="modal-cancelar" class="modal">
    <div class="modal-conteudo">
      <h3>Cancelar Atendimento</h3>
      <p>Tem certeza que deseja cancelar este atendimento?</p>
      <div class="botoes-modal">
        <button class="btn-principal" id="confirmar-cancelar">Sim</button>
        <button class="btn-cancelar" id="fechar-cancelar">Não</button>
      </div>
    </div>
  </div>

  <footer class="rodape">
    <p>© 2025 NailSpot - Seu momento de beleza e bem-estar</p>
  </footer>

  <script>
    const modalRemarcar = document.getElementById("modal-remarcar");
    const modalCancelar = document.getElementById("modal-cancelar");

    document.querySelectorAll(".btn-remarcar").forEach(btn =>
      btn.addEventListener("click", () => modalRemarcar.classList.add("mostrar"))
    );
    document.querySelectorAll(".btn-cancelar").forEach(btn =>
      btn.addEventListener("click", () => modalCancelar.classList.add("mostrar"))
    );

    document.getElementById("fechar-remarcar").addEventListener("click", () =>
      modalRemarcar.classList.remove("mostrar")
    );
    document.getElementById("fechar-cancelar").addEventListener("click", () =>
      modalCancelar.classList.remove("mostrar")
    );

    document.getElementById("form-remarcar").addEventListener("submit", e => {
      e.preventDefault();
      alert("Atendimento remarcado com sucesso!");
      modalRemarcar.classList.remove("mostrar");
    });

    document.getElementById("confirmar-cancelar").addEventListener("click", () => {
      alert("Atendimento cancelado com sucesso!");
      modalCancelar.classList.remove("mostrar");
    });
    // Controle de seleção dos serviços oferecidos
    document.getElementById('form-servicos').addEventListener('submit', e => {
      e.preventDefault();
      const manicure = document.getElementById('manicure').checked;
      const pedicure = document.getElementById('pedicure').checked;

      if (!manicure && !pedicure) {
        alert("Selecione ao menos um serviço que você oferece!");
        return;
      }

      let msg = "Serviços atualizados: ";
      if (manicure && pedicure) msg += "Manicure e Pedicure";
      else if (manicure) msg += "Manicure";
      else msg += "Pedicure";

      alert(msg);
    });
  </script>
</body>
</html>
